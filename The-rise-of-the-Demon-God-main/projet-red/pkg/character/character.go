...
// Affiche la fiche de personnage avec l'UI avancée du package ui
func (p *Personnage) AfficherFiche() {
	ui.AfficherFichePersonnage(
		p.Nom, p.Race, p.Niveau, p.NGPlus,
		p.PV, p.MaxPV, p.Mana, p.MaxMana,
		p.XP, p.MaxXP, p.Or,
		p.Attaque, p.BuffAttaque,
		p.Defense, p.BuffDefense,
		p.Dexterite,
...
func (p *Personnage) OuvrirInventaire() {
	for {
		...
		if choix == "1" {
			idxStr := ui.LireEntree("Numéro de l'objet (0 pour annuler) : ")
			var idx int
...
func (p *Personnage) UtiliserObjet(nom string, index int) {
	...
	case "Grande Potion de soin":
		...
		fmt.Println(ui.Vert + "Vous buvez une Grande Potion de soin (+100 PV)." + ui.Reset)
		p.RetirerObjetIndex(index)
	...
	case "Potion d'Elixir (Vie+Mana)":
		...
		fmt.Println(ui.Vert + "Vous buvez un Elixir (+100 PV / +100 Mana)." + ui.Reset)
		p.RetirerObjetIndex(index)

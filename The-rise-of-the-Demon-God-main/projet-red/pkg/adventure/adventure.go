		...
		mobs := []combat.Monstre{
			{
				...
				Sort:      combat.SortMonstre{Nom: "Souffle de flamme", Degats: 50, CoutMana: 25},
			...
			},
			{
				...
				GainXP: int(150 * multRecompense), GainOr: int(60 * multRecompense),
				Sort:      combat.SortMonstre{Nom: "Domaine de l'Enfer", Degats: 200, CoutMana: 60},
				TableDrop: []string{"Minerai de Mithril", "Potion d'Elixir"}, ChancDrop: 45 + (p.NGPlus * 10),
			},
		}
		m = mobs[rand.Intn(len(mobs))]
	}
...
func CombattreBossFinal(p *character.Personnage) {
	...
	boss := &combat.Monstre{
		...
		Sort:      combat.SortMonstre{Nom: "Apocalypse", Degats: int(300 * multStat), CoutMana: 100},

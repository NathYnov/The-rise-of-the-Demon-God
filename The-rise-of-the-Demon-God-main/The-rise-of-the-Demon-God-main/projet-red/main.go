...
func PlayStep(currentStep int) {
	// Vérifier si un texte de lore existe pour cette étape
	if loreText, exists := adventure.StepLore[currentStep]; exists {
		// 30ms représente une vitesse de défilement lisible
		ui.PrintLore(loreText, 30)
	}
}
